...
import image1 from '../images/fereastra-pers-1.jpg';
import image2 from '../images/fereastra-pers-2.png';
import image3 from '../images/fereastra-pers-3.jpg';
import image4 from '../images/fereastra-pers-4.png';
import image5 from '../images/fereastra-pers-5.jpg';
...
import handle3 from '../images/handlee3.png';
import '../css/customize.css';

const FereastraPersonalizata = ({ user }) => {
  // Stări pentru toate opțiunile de personalizare
  ...
  const [insectNet, setInsectNet] = useState(false);
  ...
  // Imagini pentru modelele de ferestre
  const images = [
    { src: image1, label: 'Model 1' },
    { src: image2, label: 'Model 2' },
    { src: image3, label: 'Model 3' },
    { src: image4, label: 'Model 4' },
    { src: image5, label: 'Model 5' }
  ];

  // Funcția de calculare a prețului în funcție de opțiunile selectate
  const calculatePrice = useCallback(() => {
    ...
    if (insectNet) total += 100;
    if (windowSill) total += 100;
    ...
    setPrice(total);
  }, [width, height, color, glassType, openingType, insectNet, windowSill, specialLocks, profile, handleColor]);

  // Actualizează prețul atunci când oricare dintre opțiuni se schimbă
  useEffect(() => {
    calculatePrice();
  }, [width, height, color, glassType, openingType, insectNet, windowSill, specialLocks, profile, handleColor, calculatePrice]);

  // Face scroll în partea de sus a paginii atunci când componenta se montează
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'smooth' });
  }, []);

  // Adaugă produsul personalizat în coș
  const addToCart = async () => {
    ...
    if (!user) {
      ...
      const productWithTotal = {
        ProductID: `fereastra-${model}-${width}-${height}`,
        ProductName: `Fereastra personalizata ${shoppingCart.length + 1}`,
        ProductPrice: price,
        Width: width || '',
        Height: height || '',
        Color: color || '',
        ...
        InsectNet: insectNet || false,
        WindowSill: windowSill || false,
      ...
      // Adaugă produsul în contextul coșului de cumpărături
      dispatch({ type: 'ADAUGA_IN_COS', id: `fereastra-${model}-${width}-${height}`, product: productWithTotal });
      toast.success('Fereastra personalizată a fost adăugată în coș', {
        position: "top-right",
        autoClose: 2000,
  ...
  return (
    <>
      <Helmet title='Fereastra Personalizata'>
        {/* <Navbar user={user} /> */}
        <CommonSection title='Fereastra Personalizata' />
        <Container>
          <Row>
            <Col lg='6'>
            ...
            <Col lg='6'>
              <div className="customization-form bg-light p-4 rounded mt-5">
                <h2>Customizează-ți fereastra</h2>
                <FormGroup>
                  <label>Model</label>
                  <Input
                    type="select"
                ...
                <FormGroup check>
                  <label check>
                    <Input type="checkbox" checked={insectNet} onChange={(e) => setInsectNet(e.target.checked)} />
                    <span>Plasă împotriva insectelor (+100 RON)</span>
                  </label>
                </FormGroup>
                <FormGroup check>
                  <label check>
                    <Input type="checkbox" checked={windowSill} onChange={(e) => setWindowSill(e.target.checked)} />
                    <span>Glaf (+100 RON)</span>
                  </label>
                </FormGroup>
                <FormGroup check>
                  <label check>
...
export default FereastraPersonalizata;
